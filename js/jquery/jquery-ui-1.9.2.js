/*
 * ! jQuery UI - v1.9.2 - 2013-02-20 http://jqueryui.com Includes:
 ...
 * jquery.ui.tabs.js, jquery.ui.tooltip.js Copyright (c) 2013 jQuery Foundation
...
(function ($, undefined) {
	...
	$.widget.bridge = function (name, object) {
		...
		$.fn[name] = function (options) {
			...
			options = !isMethodCall && args.length ? $.widget.extend.apply(null, [options]
				.concat(args)) : options;
			if (isMethodCall) {
				this.each(function () {
					...
					if (methodValue !== instance && methodValue !== undefined) {
						returnValue = methodValue && methodValue.jquery ? returnValue
							.pushStack(methodValue.get()) : methodValue;
						return false;
					}
				});
			} else {
	...
	$.Widget.prototype = {
		...
		_createWidget: function (options, element) {
			...
			this.options = $.widget.extend({}, this.options, this._getCreateOptions(),
				options);
			this.bindings = $();
			this.hoverable = $();
			this.focusable = $();
		...
		_setOption: function (key, value) {
			...
			if (key === "disabled") {
				this.widget().toggleClass(
					this.widgetFullName + "-disabled ui-state-disabled", !!value).attr(
						"aria-disabled", value);
				this.hoverable.removeClass("ui-state-hover");
				this.focusable.removeClass("ui-state-focus");
			}
		...
		_on: function (suppressDisabledCheck, element, handlers) {
			...
			$.each(handlers, function (event, handler) {
				...
				if (typeof handler !== "string") {
					handlerProxy.guid = handler.guid = handler.guid || handlerProxy.guid
						|| $.guid++;
				}
		...
		_trigger: function (type, event, data) {
			...
			data = data || {};
			event = $.Event(event);
			event.type = (type === this.widgetEventPrefix ? type : this.widgetEventPrefix
				+ type).toLowerCase();
	...
	$.each({
		...
		$.Widget.prototype["_" + method] = function (element, options, callback) {
			...
			var hasOptions, effectName = !options ? method : options === true
...
(function ($, undefined) {
	...
	$.widget("ui.mouse", {
		...
		_mouseDistanceMet: function (event) {
			return (Math.max(Math.abs(this._mouseDownEvent.pageX - event.pageX), Math
...
(function ($, undefined) {
	...
	$.fn.position = function (options) {
		...
		$.each(["my", "at"], function () {
			...
			if (pos.length === 1) {
				pos = rhorizontal.test(pos[0]) ? pos.concat(["center"]) : rvertical
		...
		return this.each(function () {
			var collisionPosition, using, elem = $(this), elemWidth = elem.outerWidth(), elemHeight = elem
				.outerHeight(), marginLeft = parseCss(this, "marginLeft"), marginTop = parseCss(
					this, "marginTop"), collisionWidth = elemWidth + marginLeft
						+ parseCss(this, "marginRight") + scrollInfo.width, collisionHeight = elemHeight
							+ marginTop + parseCss(this, "marginBottom") + scrollInfo.height, position = $
								.extend({}, basePosition), myOffset = getOffsets(offsets.my, elem
			...
			if (options.using) {
				...
				using = function (props) {
					var left = targetOffset.left - position.left, right = left + targetWidth
						- elemWidth, top = targetOffset.top - position.top, bottom = top
				...
				var within = data.within, withinOffset = within.isWindow
					...
					: within.offset.left, outerWidth = within.width, collisionPosLeft = position.left
						- data.collisionPosition.marginLeft, overLeft = withinOffset
							- collisionPosLeft, overRight = collisionPosLeft + data.collisionWidth
				...
				var within = data.within, withinOffset = within.isWindow
					...
					: within.offset.top, outerHeight = data.within.height, collisionPosTop = position.top
						- data.collisionPosition.marginTop, overTop = withinOffset
							- collisionPosTop, overBottom = collisionPosTop + data.collisionHeight
					...
					+ within.scrollLeft, outerWidth = within.width, offsetLeft = within.isWindow
						...
						: within.offset.left, collisionPosLeft = position.left
							- data.collisionPosition.marginLeft, overLeft = collisionPosLeft
								- offsetLeft, overRight = collisionPosLeft + data.collisionWidth
									- outerWidth - offsetLeft, myOffset = data.my[0] === "left"
										? -data.elemWidth
			...
			top: function (position, data) {
				var within = data.within, withinOffset = within.offset.top + within.scrollTop, outerHeight = within.height, offsetTop = within.isWindow
					...
					: within.offset.top, collisionPosTop = position.top
						- data.collisionPosition.marginTop, overTop = collisionPosTop - offsetTop, overBottom = collisionPosTop
				...
				if (overTop < 0) {
					...
					newOverTop = position.top - data.collisionPosition.marginTop + myOffset
	...
	$.widget("ui.draggable", $.ui.mouse, {
		...
		_getHandle: function (event) {
			var handle = !this.options.handle
				|| !$(this.options.handle, this.element).length ? true : false;
		...
		_getParentOffset: function () {
			...
			if ((this.offsetParent[0] == document.body) // This needs to be actually
				// done for all browsers, since
				// pageX/pageY includes this
				// information
				|| (this.offsetParent[0].tagName
					&& this.offsetParent[0].tagName.toLowerCase() == 'html' && $.ui.ie)) // Ugly
				// IE
				// fix
				po = {
					top: 0,
		...
		_convertPositionTo: function (d, pos) {
			...
			return {
				top: (pos.top // The absolute mouse position
					...
					// Relative offset from element to
					// offset parent
				...
				left: (pos.left // The absolute mouse position
					+ this.offset.relative.left * mod // Only for relative positioned
		...
		_generatePosition: function (event) {
			...
			if (this.originalPosition) { // If we are not dragging yet, we won't check
				...
				if (this.containment) {
					if (this.relative_container) {
						...
						this.containment[1] + co.top, this.containment[2] + co.left,
			...
			return {
				// 2021年7月12日15:38:16 解决放大后拖拽组件移动不同步 zh
				top: (pageY // The absolute mouse position
					- this.offset.click.top // Click offset (relative to the element)
					- this.offset.relative.top // Only for relative positioned nodes:
					// Relative offset from element to offset
					// parent
					- this.offset.parent.top // The offsetParent's offset without borders
					// (offset + border)
					+ ((this.cssPosition == 'fixed'
						? -this.scrollParent.scrollTop()
						: (scrollIsRootNode ? 0 : scroll.scrollTop())))) / magnification,
				left: (pageX // The absolute mouse position
					...
					+ ((this.cssPosition == 'fixed'
						? -this.scrollParent.scrollLeft()
						: scrollIsRootNode ? 0 : scroll.scrollLeft()))) / magnification
			};
		},
		_clear: function () {
		...
		_trigger: function (type, event, ui) {
			...
			if (type == "drag")
				this.positionAbs = this._convertPositionTo("absolute"); // The absolute
			// position has to
			// be recalculated
			// after plugins
		...
		start: function (event, ui) {
			...
			$(o.connectToSortable).each(function () {
				...
				if (sortable && !sortable.options.disabled) {
					...
					// refreshPositions at drag start to
					// refresh the containerCache since
					// the sortable container cache is
					// used in drag and needs to be up
					// to date (this will ensure it's
					// initialised as well as being kept
					// in step with any changes that
		...
		stop: function (event, ui) {
			...
			$.each(inst.sortables, function () {
				if (this.instance.isOver) {
					...
					// sortable instance (so
					// sortable plugins like
					// revert still work)
					// The sortable revert is supported, and we have to set a
					// temporary dropped variable on the draggable to support revert:
					// 'valid/invalid'
					...
					this.instance.cancelHelperRemoval = false; // Remove the helper in
			...
			$.each(inst.sortables, function (i) {
				...
				if (this.instance._intersectsWith(this.instance.containerCache)) {
					...
					$.each(inst.sortables, function () {
						...
						if (this != thisSortable
							...
							&& $.ui.contains(thisSortable.instance.element[0],
				...
				if (innermostIntersecting) {
					...
					if (!this.instance.isOver) {
						...
						// by cloning the list group item, appending it to the sortable
						// and using it as inst.currentItem
						// We can then fire the start event of the sortable with our
						// passed browser event, and our own helper (so it doesn't
						// create a new one)
						this.instance.currentItem = $(that).clone().removeAttr('id')
						...
						inst.dropped = this.instance.element; // draggable revert needs
						// that
					...
					if (this.instance.isOver) {
						...
						this.instance.options.helper = this.instance.options._helper;
						// Now we remove our currentItem, the list group clone again,
						// and the placeholder, and animate the helper back to it's
						// original size
	...
	$.ui.plugin.add("draggable", "snap", {
		...
		drag: function (event, ui) {
			...
			for (var i = inst.snapElements.length - 1; i >= 0; i--) {
				...
				if (!((l - d < x1 && x1 < r + d && t - d < y1 && y1 < b + d) || (l - d < x1 && x1 < r + d && t - d < y2 && y2 < b + d)
					|| (l - d < x2 && x2 < r + d && t - d < y1 && y1 < b + d) || (l - d < x2 && x2 < r + d && t - d < y2 && y2 < b + d))) {
	...
	$.ui.ddmanager = {
		...
		prepareOffsets: function (t, event) {
			...
			droppablesLoop: for (var i = 0; i < m.length; i++) {
				...
				if (type == "mousedown")
					m[i]._activate.call(m[i], event); // Activate the droppable if used
				// directly from draggables
		...
		drag: function (draggable, event) {
			...
			$.each($.ui.ddmanager.droppables[draggable.options.scope] || [], function () {
				if (this.options.disabled || this.greedyChild || !this.visible)
					return;
				var intersects = $.ui
					.intersect(draggable, this, this.options.tolerance);
...
(function ($, undefined) {
	$.widget("ui.resizable", $.ui.mouse, {
		...
		_create: function () {
			...
			if (this.handles.constructor == String) {
				...
				for (var i = 0; i < n.length; i++) {
					var hclass = $.trim(n[i]).split(".");
					var handle = hclass[0], klass = hclass[1], hname = 'ui-resizable-' + handle;
			...
			this._renderAxis = function (target) {
				...
				for (var i in this.handles) {
					...
					if (this.elementIsWrapper
						...
						padWrapper = /sw|ne|nw|se|n|s/.test(i) ? axis.outerHeight() : axis
							.outerWidth();
				...
				if (!that.resizing) {
					if (this.className)
						var axis = this.className
							.match(/ui-resizable-(p1|p2|se|sw|ne|nw|n|e|s|w)/i);
					// Axis, default = se
		...
		_rotate(cx, cy, x, y, counter) {
			var calc = this.calc[counter === -1 ? 1 : 0];
			var nx = (calc.cos * (x - cx)) + (calc.sin * (y - cy)) + cx;
			var ny = (calc.cos * (y - cy)) - (calc.sin * (x - cx)) + cy;
			return { x: nx, y: ny };
		},
		_mouseStart: function (event) {
			...
			// if (false && el.is('.ui-draggable') || (/absolute/).test(el.css('position'))) {
			// 	el.css({
			// 		position: 'absolute',
			// 		top: iniPos.top,
			// 		left: iniPos.left
			...
			this.x0 = this.originalPosition.left;
			this.y0 = this.originalPosition.top;
			this.x1 = this.originalPosition.left + this.originalSize.width;
			this.y1 = this.originalPosition.top + this.originalSize.height;
			this.rad = this.element.rad();
			if (this.rad) {
				this.calc = [{ cos: Math.cos(-this.rad), sin: Math.sin(-this.rad) },
				{ cos: Math.cos(this.rad), sin: Math.sin(this.rad) }];

				var cx = (this.x0 + this.x1) / 2,
					cy = (this.y0 + this.y1) / 2;
				this.rp0 = this._rotate(cx, cy, this.x0, this.y0, 1);
				this.rp1 = this._rotate(cx, cy, this.x1, this.y1, 1);
				this.rotated = {
					x0: this.rp0.x,
					y0: this.rp0.y,
					x1: this.rp1.x,
				...
				var p0 = this._rotate(cx, cy, this.rotated.x0, this.rotated.y0, -1);

				// if (!this.element.parent().find('.no-rotated-start').length) {
				// 	$("<div>").addClass('no-rotated-start').appendTo(this.element.parent());
				...
				var data = {
					...
					transform: "rotate({0}rad)".format(this.rad)
				}

				// this.element.parent().find('.no-rotated-start').css(data)
			}
			// Aspect Ratio
		...
		_mouseDrag: function (event) {
			...
			// 2021年7月9日10:36:38 解决放大后拖拽放大组件不同步
			dx = getRound(calculate(dx, '/', magnification), 0);
			dy = getRound(calculate(dy, '/', magnification), 0);
			...
			// }
			var data, dw = 0, dh = 0,
				points = trigger.apply(this, [event, this.rad ? this.rotated : this, dx, dy]);
			...
			if (this.rad) {
				...
				var cx = (points.x0 + points.x1) / 2,
					cy = (points.y0 + points.y1) / 2,
					p0 = this._rotate(cx, cy, points.x0, points.y0, -1),
					p1 = this._rotate(cx, cy, points.x1, points.y1, -1);
				points = {
					x0: p0.x,
					y0: p0.y,
					x1: p1.x,
				...
				dw = this.sizeDiff.width;
				dh = this.sizeDiff.height;


			}
			data = this.axis[0] == 'p' ? points : {
				left: Math.min(points.x0, points.x1),
				top: Math.min(points.y0, points.y1),
				width: Math.abs(points.x0 - points.x1) + dw,
				height: Math.abs(points.y0 - points.y1) + dh
			}
			...
			if (this._helper) {
				var pr = this._proportionallyResizeElements, ista = pr.length
					&& (/textarea/i).test(pr[0].nodeName), soffseth = ista
						...
						? 0
						: that.sizeDiff.height, soffsetw = ista ? 0 : that.sizeDiff.width;
				var s = {
		...
		_respectSize: function (data, event) {
			var el = this.helper, o = this._vBoundaries, pRatio = this._aspectRatio
				|| event.shiftKey, a = this.axis, ismaxw = isNumber(data.width) && o.maxWidth
		...
		_proportionallyResize: function () {
			...
			for (var i = 0; i < this._proportionallyResizeElements.length; i++) {
				...
				if (!this.borderDif) {
					...
					prel.css('borderBottomWidth'), prel.css('borderLeftWidth')], p = [
		...
		_change: {
			p: function (e, p1, p2) {
				...
				this.element.find("line").attr(p1p2_);
				this.options.handlesChanged(this.element);
				return {
					left: x,
					top: y,
			...
			p1: function (e, dx, dy) {
				dx = e.pageX - this.originalMousePosition.left;
				dy = e.pageY - this.originalMousePosition.top;

				var p1p2 = this.element.data("p1p2");
				var p1 = { x: p1p2.p1.x + dx, y: p1p2.p1.y + dy };

				return this._change['p'].apply(this, [e, p1, p1p2.p2]);
			},
			p2: function (e, dx, dy) {
				dx = e.pageX - this.originalMousePosition.left;
				dy = e.pageY - this.originalMousePosition.top;
				...
				var p1p2 = this.element.data("p1p2");
				var p2 = { x: p1p2.p2.x + dx, y: p1p2.p2.y + dy };
				return this._change['p'].apply(this, [e, p1p2.p1, p2]);
			},
			e: function (e, p01, dx, dy) {
				if (this.rad) {
					var cos = this.calc[0].cos;
					var sin = this.calc[0].sin;
					var r = -dy / cos;
					dx = dx + r * sin;
					r = dx * cos;
					var x1 = p01.x1 + cos * r;
			...
			s: function (e, p01, dx, dy) {
				if (this.rad) {
					var cos = this.calc[0].cos;
					var sin = this.calc[0].sin;
					var r = dx / cos;
					dy = dy + r * sin;
					var x1 = p01.x1 + sin * cos * dy;
					var y1 = p01.y1 + cos * cos * dy;
					return {
						x0: p01.x0,
						y0: p01.y0,
						x1: x1,
			...
			sw: function (event, dx, dy) {
				return $.extend(this._change.s.apply(this, arguments), this._change.w
					.apply(this, [event, dx, dy]));
			},
			ne: function (event, dx, dy) {
				return $.extend(this._change.n.apply(this, arguments), this._change.e
					.apply(this, [event, dx, dy]));
			},
			nw: function (event, dx, dy) {
				return $.extend(this._change.n.apply(this, arguments), this._change.w
					.apply(this, [event, dx, dy]));
			}
		},
	...
	$.ui.plugin.add("resizable", "alsoResize", {
		...
		resize: function (event, ui) {
			...
			var delta = {
				...
				$(exp).each(function () {
					var el = $(this), start = $(this).data("resizable-alsoresize"), style = {}, css = c
						&& c.length ? c : el.parents(ui.originalElement[0]).length ? ['width',
	...
	$.ui.plugin.add("resizable", "containment", {
		start: function (event, ui) {
			...
			// i'm a node, so compute top, left, right, bottom
				...
				var co = that.containerOffset, ch = that.containerSize.height, cw = that.containerSize.width, width = ($.ui
					.hasScroll(ce, "left") ? ce.scrollWidth : cw), height = ($.ui.hasScroll(ce)
						? ce.scrollHeight
						: ch);
				that.parentData = {
					element: ce,
		...
		resize: function (event, ui) {
			...
			var woset = Math.abs((that._helper
				? that.offset.left - cop.left
				: (that.offset.left - cop.left))
				+ that.sizeDiff.width), hoset = Math.abs((that._helper ? that.offset.top
		...
		stop: function (event, ui) {
			...
			var helper = $(that.helper), ho = helper.offset(), w = helper.outerWidth()
				- that.sizeDiff.width, h = helper.outerHeight() - that.sizeDiff.height;
...
(function ($, undefined) {
	$.widget("ui.selectable", $.ui.mouse, {
		...
		_mouseStart: function (event) {
			...
			if (this.options.disabled || window["mydraggable-disabled"])
				return;
		...
		_mouseDrag: function (event) {
			...
			this.selectees.each(function () {
				...
				if (hit) {
					...
					if (true || !selectee.$element.is('.ui-selecting')) {
		...
		_mouseStop: function (event) {
			...
			$('.ui-selecting', this.element[0]).each(function () {
				var selectee = $.data(this, "selectable-item");
				selectee.$element
					.addClass('ui-selected').removeClass('ui-selecting');
				selectee.selecting = false;
				selectee.selected = true;
				selectee.startselected = true;
...
(function ($, undefined) {
	$.widget("ui.sortable", $.ui.mouse, {
		...
		_mouseStart: function (event, overrideHandle, noActivation) {
			...
			// not to be visible before getting its correct
			// position
			return true;
		},
		_mouseDrag: function (event) {
			...
			for (var i = this.items.length - 1; i >= 0; i--) {
				...
				if (itemElement != this.currentItem[0] // cannot intersect with itself
					...
					// done
					// before
					&& !$.contains(this.placeholder[0], itemElement) // no action if the
					// item moved is the
					// parent of the
					// item checked
		...
		_intersectsWith: function (item) {
			...
			var dyClick = this.offset.click.top, dxClick = this.offset.click.left;
			var isOverElement = (y1 + dyClick) > t && (y1 + dyClick) < b && (x1 + dxClick) > l
				&& (x1 + dxClick) < r;
		...
		_intersectsWithPointer: function (item) {
			var isOverElementHeight = (this.options.axis === 'x')
				|| $.ui.isOverAxis(this.positionAbs.top + this.offset.click.top,
					item.top, item.height), isOverElementWidth = (this.options.axis === 'y')
						|| $.ui.isOverAxis(this.positionAbs.left + this.offset.click.left,
		...
		_intersectsWithSides: function (item) {
			var isOverBottomHalf = $.ui.isOverAxis(this.positionAbs.top
				+ this.offset.click.top, item.top + (item.height / 2), item.height), isOverRightHalf = $.ui
		...
		_getItemsAsjQuery: function (connected) {
						...
						if (inst && inst != this && !inst.options.disabled) {
							queries.push([
								$.isFunction(inst.options.items) ? inst.options.items
									.call(inst.element) : $(inst.options.items, inst.element)
			...
			queries.push([
				...
				}) : $(this.options.items, this.element).not(".ui-sortable-helper")
			...
			if (!o.placeholder || o.placeholder.constructor == String) {
				...
				o.placeholder = {
					...
					update: function (container, p) {
						...
						// styles coming from a stylesheet), it receives the inline height
		...
		_contactContainers: function (event) {
			...
			if (this.containers.length === 1) {
				...
				itemWithLeastDistance ? this._rearrange(event, itemWithLeastDistance, null,
					true) : this._rearrange(event, null,
	...
	$.widget("ui.accordion", {
		...
		_create: function () {
			...
			this.headers.attr("role", "tab").each(function (i) {
				var header = $(this), headerId = header.attr("id"), panel = header.next(), panelId = panel
					.attr("id");
				if (!headerId) {
		...
		_createIcons: function () {
			...
			if (icons) {
				$("<span>").addClass("ui-accordion-header-icon ui-icon " + icons.header)
		...
		(function ($, prototype) {
			...
			prototype._create = function () {
				if (this.options.navigation) {
					var that = this, headers = this.element.find(this.options.header), content = headers
...
(function ($, undefined) {
	...
	$.widget("ui.autocomplete", {
		...
		_create: function () {
					...
					switch (event.keyCode) {
						...
						case keyCode.ESCAPE:
							if (this.menu.element.is(":visible")) {
								...
								this.close(event);
								// Different browsers have different default behavior for
								// escape
			...
			this._initSource();
			this.menu = $("<ul>").addClass("ui-autocomplete").appendTo(this.document
			...
			this._on(this.menu.element, {
				...
				menufocus: function (event, ui) {
					...
					if (false !== this._trigger("focus", event, {
						...
						// menu is navigated, causing screen readers to notice a change
						// and
						// announce the item. Since the focus event was canceled, this
						// doesn't
						// happen, so we update the live region so that screen readers
						// can
				...
				menuselect: function (event, ui) {
					...
					// TODO remove the fallback, see #8156
					var item = ui.item.data("ui-autocomplete-item")
						|| ui.item.data("item.autocomplete"), previous = this.previous;
	...
	$.widget("ui.button", {
		...
		_create: function () {
			...
			if (toggleButton) {
				...
				// if mouse moves between mousedown and mouseup (drag) set clickDragged
				// flag
				// prevents issue where button state changes but checkbox/radio checked
				// state
		...
		_destroy: function () {
			...
			this.buttonElement.removeClass(baseClasses + " " + stateClasses + " "
				+ typeClasses).removeAttr("role").removeAttr("aria-pressed")
		...
		refresh: function () {
			...
			var isDisabled = this.element.is("input, button") ? this.element
				.is(":disabled") : this.element.hasClass("ui-button-disabled");
		...
		_resetButton: function () {
			...
			var buttonElement = this.buttonElement.removeClass(typeClasses), buttonText = $(
				"<span></span>", this.document[0]).addClass("ui-button-text")
				.html(this.options.label).appendTo(buttonElement.empty()).text(), icons = this.options.icons, multipleIcons = icons.primary
	...
	$.widget("ui.buttonset", {
		...
		refresh: function () {
			...
			this.buttons = this.element.find(this.options.items).filter(":ui-button")
				...
				.filter(":first").addClass(rtl ? "ui-corner-right" : "ui-corner-left")
				.end().filter(":last").addClass(rtl
					? "ui-corner-left"
...
(function ($, undefined) {
	...
	function Datepicker() {
		...
		this.regional[''] = { // Default regional settings
			...
			monthNames: ['January', 'February', 'March', 'April', 'May', 'June', 'July',
				'August', 'September', 'October', 'November', 'December'], // Names of
			// months for
			// drop-down
			// and
			// formatting
			...
			showMonthAfterYear: false, // True if the year select precedes month, false
			// for month then year
		...
		this._defaults = { // Global defaults for all the date picker instances
			...
			// only prev/next
			changeYear: false, // True if year can be selected directly, false if only
			// prev/next
			...
			// with
			// [0] = true if selectable, false if not, [1] = custom CSS class name(s)
			// or '',
			...
			showCurrentAtPos: 0, // The position in multipe months at which to show the
	...
	$.extend(Datepicker.prototype, {
		...
		_attachments: function (input, inst) {
			...
			if (showOn == 'focus' || showOn == 'both') // pop-up date picker when in the
				// marked field
				input.focus(this._showDatepicker);
			if (showOn == 'button' || showOn == 'both') { // pop-up date picker when button
				// clicked
		...
		_optionDatepicker: function (target, name, value) {
			...
			if (arguments.length == 2 && typeof name == 'string') {
				return (name == 'defaults' ? $.extend({}, $.datepicker._defaults) : (inst
		...
		_doKeyDown: function (event) {
			...
			if ($.datepicker._datepickerShowing)
				switch (event.keyCode) {
					...
					case 33:
						$.datepicker._adjustDate(event.target, (event.ctrlKey ? -$.datepicker
							._get(inst, 'stepBigMonths') : -$.datepicker._get(inst,
								'stepMonths')), 'M');
			...
			if (inst.input.val() != inst.lastVal) {
				try {
					var date = $.datepicker.parseDate($.datepicker._get(inst, 'dateFormat'),
						(inst.input ? inst.input.val() : null), $.datepicker
		...
		parseDate: function (format, value, settings) {
			...
			var getNumber = function (match) {
				...
				var size = (match == '@' ? 14 : (match == '!' ? 20 : (match == 'y' && isDoubled
					? 4
					: (match == 'o' ? 3 : 2))));
					...
					if (literal)
						...
						switch (format.charAt(iFormat)) {
							...
							case 'D':
								...
								break;
							case 'o':
								output += formatNumber('o', Math.round((new Date(date
									.getFullYear(), date.getMonth(), date.getDate())
									.getTime() - new Date(date.getFullYear(), 0, 0).getTime())
									/ 86400000), 3);
								break;
							case 'm':
		...
		_getDefaultDate: function (inst) {
			return this._restrictMinMax(inst, this._determineDate(inst, this._get(inst,
		...
		_determineDate: function (inst, date, defaultDate) {
			...
			var newDate = (date == null || date === '' ? defaultDate : (typeof date == 'string'
				? offsetString(date)
				: (typeof date == 'number'
		...
		_attachHandlers: function (inst) {
			...
			inst.dpDiv.find('[data-handler]').map(function () {
				var handler = {
					prev: function () {
						window['DP_jQuery_' + dpuuid].datepicker._adjustDate(id, -stepMonths,
							'M');
					},
					next: function () {
						window['DP_jQuery_' + dpuuid].datepicker._adjustDate(id, +stepMonths,
					...
					selectDay: function () {
						window['DP_jQuery_' + dpuuid].datepicker._selectDay(id, +this
		...
		_generateHTML: function (inst) {
			var today = new Date();
			today = this._daylightSavingAdjust(new Date(today.getFullYear(), today
				.getMonth(), today.getDate())); // clear time
			var isRTL = this._get(inst, 'isRTL');
			...
			if (maxDate) {
				var maxDraw = this
					._daylightSavingAdjust(new Date(maxDate.getFullYear(), maxDate
						.getMonth()
			...
			prevText = (!navigationAsDateFormat ? prevText : this
				.formatDate(prevText, this._daylightSavingAdjust(new Date(drawYear,
					drawMonth - stepMonths, 1)), this._getFormatConfig(inst)));
			...
			nextText = (!navigationAsDateFormat ? nextText : this
				.formatDate(nextText, this._daylightSavingAdjust(new Date(drawYear,
					drawMonth + stepMonths, 1)), this._getFormatConfig(inst)));
			...
			for (var row = 0; row < numMonths[0]; row++) {
				...
				for (var col = 0; col < numMonths[1]; col++) {
					...
					var numRows = (isMultiMonth
						...
						: curRows); // If multiple months, use the higher number of rows
					// (see #7043)
					this.maxRows = numRows;
					var printDate = this._daylightSavingAdjust(new Date(drawYear, drawMonth,
						1 - leadDays));
					for (var dRow = 0; dRow < numRows; dRow++) { // create date picker rows
						...
						for (var dow = 0; dow < 7; dow++) { // create date picker days
							var daySettings = (beforeShowDay ? beforeShowDay.apply((inst.input
								? inst.input[0]
								: null), [printDate]) : [true, '']);
							var otherMonth = (printDate.getMonth() != drawMonth);
							var unselectable = (otherMonth && !selectOtherMonths)
								|| !daySettings[0] || (minDate && printDate < minDate)
							...
							tbody += '<td class="'
								...
								((printDate.getTime() == selectedDate.getTime()
									...
									// or defaultDate is current printedDate and defaultDate
									// is selectedDate
									' ' + this._dayOverClass
									: '')
		...
		_adjustInstDate: function (inst, offset, period) {
			...
			var day = Math.min(inst.selectedDay, this._getDaysInMonth(year, month))
				+ (period == 'D' ? offset : 0);
			var date = this._restrictMinMax(inst, this._daylightSavingAdjust(new Date(
				year, month, day)));
			inst.selectedDay = date.getDate();
			inst.drawMonth = inst.selectedMonth = date.getMonth();
			inst.drawYear = inst.selectedYear = date.getFullYear();
	 ...
	 * only occurs once in the lifetime of the parent div. Global instActive, set
	 * by _updateDatepicker allows the handlers to find their way back to the
	 * active picker.
	 */
	...
	$.fn.datepicker = function (options) {
		...
		return this.each(function () {
			typeof options == 'string' ? $.datepicker['_' + options + 'Datepicker']
				.apply($.datepicker, [this].concat(otherArgs)) : $.datepicker
...
(function ($, undefined) {
	...
	$.widget("ui.dialog", {
		...
		_create: function () {
			...
			uiDialog = (this.uiDialog = $("<div>")).addClass(uiDialogClasses
			...
			this.element.show().removeAttr("title")
				.addClass("ui-dialog-content ui-widget-content").appendTo(uiDialog);
			uiDialogTitlebar = (this.uiDialogTitlebar = $("<div>"))
				.addClass("ui-dialog-titlebar  ui-widget-header  "
					+ "ui-corner-all  ui-helper-clearfix").bind("mousedown",
						function () {
							// Dialog isn't getting focus when dragging (#8063)
							uiDialog.focus();
						}).prependTo(uiDialog);
			uiDialogTitlebarClose = $("<a href='#'></a>")
				.addClass("ui-dialog-titlebar-close  ui-corner-all").attr("role",
					"button").click(function (event) {
						event.preventDefault();
						that.close(event);
					}).appendTo(uiDialogTitlebar);
			(this.uiDialogTitlebarCloseText = $("<span>"))
				.addClass("ui-icon ui-icon-closethick").text(options.closeText)
			...
			this._on(uiDialog, {
				keydown: function (event) {
					...
					var tabbables = $(":tabbable", uiDialog), first = tabbables
		...
		_destroy: function () {
			...
			this.uiDialog.hide();
			this.element.removeClass("ui-dialog-content ui-widget-content").hide()
				.appendTo("body");
		...
		_size: function () {
			 ...
			 * .ui-dialog-content divs will both have width and height set, so we need
			 * to reset them
			 */
	...
	$.extend($.ui.dialog.overlay, {
				...
				setTimeout(function () {
					...
					if ($.ui.dialog.overlay.instances.length) {
						$(document).bind($.ui.dialog.overlay.events,
							function (event) {
								// stop events if the z-index of the target is < the
								// z-index of the overlay
								// we cannot return true when we don't want to cancel
								// the event (#3523)
		...
		resize: function () {
			 ...
			 * stretch the overlay. If the user then drags the dialog back to the
			 * left, the document will become narrower, so we need to shrink the
			 * overlay to the appropriate size. This is handled by shrinking the
			 * overlay before setting it to the full document size.
			 */
			var $overlays = $([]);
...
(function ($, undefined) {
	...
	$.widget("ui.menu", {
		...
		_keydown: function (event) {
			...
			switch (event.keyCode) {
				...
				default:
					...
					match = skip && match.index(this.active.next()) !== -1 ? this.active
						.nextAll(".ui-menu-item") : match;
		...
		refresh: function () {
			...
			menus.children(":not(.ui-menu-item):has(a)").addClass("ui-menu-item")
				.attr("role", "presentation").children("a").uniqueId()
				.addClass("ui-corner-all").attr({
					tabIndex: -1,
					role: this._itemRole()
				});
			// Initialize unlinked menu-items containing spaces and/or dashes only as
			// dividers
		...
		_open: function (submenu) {
			...
			clearTimeout(this.timer);
			this.element.find(".ui-menu").not(submenu.parents(".ui-menu")).hide()
		...
		expand: function (event) {
			...
			if (newItem && newItem.length) {
				...
				// Delay so Firefox will not hide activedescendant change in expanding
				// submenu from AT
				this._delay(function () {
					this.focus(event, newItem);
				});
	...
	$.widget("ui.slider", $.ui.mouse, {
		...
		_create: function () {
			var i, handleCount, o = this.options, existingHandles = this.element
				.find(".ui-slider-handle").addClass("ui-state-default ui-corner-all"), handle = "<a class='ui-slider-handle ui-state-default ui-corner-all' href='#'></a>", handles = [];
		...
		_destroy: function () {
			...
			this.element.removeClass("ui-slider" + " ui-slider-horizontal"
				+ " ui-slider-vertical" + " ui-slider-disabled" + " ui-widget"
		...
		_refreshValue: function () {
			...
			if (this.options.values && this.options.values.length) {
				...
				valPercent = (valueMax !== valueMin) ? (value - valueMin) / (valueMax - valueMin)
					* 100 : 0;
	...
	$.widget("ui.spinner", {
		...
		_draw: function () {
			var uiSpinner = this.uiSpinner = this.element.addClass("ui-spinner-input")
				.attr("autocomplete", "off").wrap(this._uiSpinnerHtml()).parent()
				// add buttons
				.append(this._buttonHtml());
			this.element.attr("role", "spinbutton");
			// button bindings
			this.buttons = uiSpinner.find(".ui-spinner-button").attr("tabIndex", -1)
		...
		_buttonHtml: function () {
			return "" + "<a class='ui-spinner-button ui-spinner-up ui-corner-tr'>"
				+ "<span class='ui-icon " + this.options.icons.up + "'>&#9650;</span>"
				+ "</a>" + "<a class='ui-spinner-button ui-spinner-down ui-corner-br'>"
		...
		_increment: function (i) {
			...
			if (incremental) {
				return $.isFunction(incremental) ? incremental(i) : Math.floor(i * i * i / 50000
					- i * i / 500 + 17 * i / 200 + 1);
			}
			return 1;
		},
		...
		_parse: function (val) {
			if (typeof val === "string" && val !== "") {
				val = window.Globalize && this.options.numberFormat ? Globalize.parseFloat(
					val, 10, this.options.culture) : +val;
			}
		...
		_destroy: function () {
			this.element.removeClass("ui-spinner-input").prop("disabled", false)
	...
	$.widget("ui.tabs", {
		...
		_create: function () {
			...
			if ($.isArray(options.disabled)) {
				options.disabled = $.unique(options.disabled.concat($.map(this.tabs
		...
		_tabKeydown: function (event) {
			...
			if (!event.ctrlKey) {
				// Update aria-selected immediately so that AT think the tab is already
				// selected.
		...
		_processTabs: function () {
			...
			this.anchors.each(function (i, anchor) {
				var selector, panel, panelId, anchorId = $(anchor).uniqueId().attr("id"), tab = $(anchor)
					.closest("li"), originalAriaControls = tab.attr("aria-controls");
				// inline tab
				if (isLocal(anchor)) {
		...
		_createPanel: function (id) {
			return $("<div>").attr("id", id)
				.addClass("ui-tabs-panel ui-widget-content ui-corner-bottom").data(
		...
		_toggle: function (event, eventData) {
			...
			// If we're opening from collapsed state, remove the previous tab from the
			// tab order.
		...
		_getIndex: function (index) {
			// meta-function to give users option to provide a href string instead of
			// a numerical index.
			...
			this.tabs.add(this.panels).each(function () {
				if ($.data(this, "ui-tabs-destroy")) {
					...
					$(this)
						...
						.removeAttr("aria-busy").removeAttr("aria-selected")
	...
	if ($.uiBackCompat !== false) {
			...
			_ajaxSettings: function (anchor, event, ui) {
				...
				return $.extend({}, ajaxOptions, {
					error: function (xhr, status) {
						try {
							// Passing index avoid a race condition when this method
							// is
							...
							ajaxOptions.error(xhr, status,
								ui.tab.closest("li").index(), ui.tab[0]);
						} catch (error) {
						}
					}
				}, this._superApply(arguments));
		...
		$.widget("ui.tabs", $.ui.tabs, {
			...
			add: function (url, label, index) {
				...
				var doInsertAfter, panel, options = this.options, li = $(options.tabTemplate
					.replace(/#\{href\}/g, url).replace(/#\{label\}/g, label)), id = !url
						.indexOf("#") ? url.replace("#", "") : this._tabId(li);
			...
			remove: function (index) {
				index = this._getIndex(index);
				var options = this.options, tab = this.tabs.eq(index).remove(), panel = this
		...
		$.widget("ui.tabs", $.ui.tabs, {
			...
			_tabId: function (tab) {
				...
				return $(a).closest("li").attr("aria-controls")
					|| a.title
					&& a.title.replace(/\s/g, "_")
						.replace(/[^\w\u00c0-\uFFFF\-]/g, "") || this.options.idPrefix
					+ getNextTabId();
			}
		});
		// _createPanel method
		...
		$.widget("ui.tabs", $.ui.tabs, {
			select: function (index) {
				...
				this.anchors.eq(index).trigger(this.options.event
					+ this.eventNamespace);
			}
		});
		// cookie option
		...
		// If a user wants to use the new animation API, they must give up the old
		// API.
		$.widget("ui.tabs", $.ui.tabs, {
			options: {
				fx: null
...
(function ($) {
	...
	function removeDescribedBy(elem) {
		var id = elem.data("ui-tooltip-id"), describedby = (elem
			.attr("aria-describedby") || "").split(/\s+/), index = $.inArray(id,
				describedby);
		if (index !== -1) {
	...
	$.widget("ui.tooltip", {
		...
		close: function (event) {
			var that = this, target = $(event ? event.currentTarget : this.element), tooltip = this
				._find(target);
